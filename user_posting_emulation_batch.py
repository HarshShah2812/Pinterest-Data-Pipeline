...
class PostData():
    '''
    A class that posts data to the batch AWS service
    
    Attributes
    ----------
    headers: dict
        The HTTP headers necessary for the API to process the batch data request
    
    Methods
    -------
    __create_payload(data_structure):
        creates the payload for the batch data to be sent to AWS MSK
    
    send_request(data_structure, record_type):
        sends a POST request to the API for the data to be sent to the Kafka topic
    
    post_data(pin_data_structure, geo_data_structure, user_data_structure):
        sends batch data requests to API in order to be sent to the corresponding Kafka topic
    '''  
    ...
    def __create_payload(self, data_structure):
        '''
        Parameters
        ----------
        data_structure: dict
        ...
        -------
        payload: json
            configures the data into the json format, so that it's ready to send to the Kafka topic
        '''    
        payload = json.dumps({
                "records": [
                    {
    ...
    def send_request(self, data_structure, record_type):
        '''
        Parameters
        ----------
        data_structure: dict
        ...
        record_type: str
            the type of record being added to the dataframe
        '''
        invoke_url = f"https://w65ht1cpna.execute-api.us-east-1.amazonaws.com/REST/topics/0e4c2ab6fb3b.{record_type}"
        payload = self.__create_payload(data_structure)
        response = requests.request("POST", invoke_url, headers = self.headers, data = payload)
        print(f"{record_type}:{response.status_code}")

    def post_data(self, pin_data_structure, geo_data_structure, user_data_structure):
        '''
        Sends batch data requests to the API, which then sends them to their respective topics
        ...
        geo_data_structure: dict
            structure of the geo_result provided by the user posting emulation
        '''
        self.send_request(pin_data_structure, "pin")
        self.send_request(geo_data_structure, "geo")
        self.send_request(user_data_structure, "user")
    ...
    while True:
        ...
        with engine.connect() as connection:
            ...
            pin_data_structure = {
                                  ...
                                  "downloaded": upe.pin_result["downloaded"], "save_location": upe.pin_result["save_location"], "category": upe.pin_result["category"]}
            ...
            user_data_structure = {
                        #Data should be sent as pairs of column_name:value, with different columns separated by commas
                        "ind": upe.user_result["ind"], "first_name": upe.user_result["first_name"], 
                        "last_name": upe.user_result["last_name"], "age": upe.user_result["age"], "date_joined": upe.user_result["date_joined"].strftime("%Y-%m-%d %H:%M:%S")}

            pd.post_data(pin_data_structure, geo_data_structure, user_data_structure)
